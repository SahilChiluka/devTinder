...
## Query Params
http://localhost:7777/user?firstName=Sahil&lastName=Chiluka
Explanation: query params are the key value pairs that are sent in the URL after the "?" symbol. 
They are used to send data to the server. In this case, we are sending firstName and lastName as query params. 
We can access them using req.query object.
...
## Route Params
http://localhost:7777/user/Sahil/Chiluka
Explanation: route params are the parameters that are sent in the URL as part of the path. 
They are used to identify a specific resource. In this case, we are sending firstName and lastName as route params. 
